...
Exhaust definition (for per-POC "please exhaust" email):
─────────────────────────────────────────────────────────────────────────────
  An account is considered EXHAUSTED (properly worked) when it has ANY of:
    • MQL / Activation / SQL / DCB contacts  (pipeline value exists)
    • 3+ contacts in CNC stages              (tried many people, can't connect)
    • 3+ contacts in NOI stage               (tried many people, not interested)
  NOT yet exhausted = none of the above → POC email lists it as "needs work"
...
Email schedule:
  ...
  • Weekly workflow (account_health_weekly.yml):
      - Team digest → cc list (management overview)
      - Per-POC email → each BD rep (their own unexhausted accounts)
"""
import argparse
import json
import os
...
def compute_health(contacts: list) -> dict:
    ...
    for ct in contacts:
        ...
        e = by_co.setdefault(co_id, {
            ...
            "cnc": 0,   # contacts in CNC stages specifically
            "mql": 0, "hot": 0,
            "terminal": 0, "noi": 0,
    ...
    for e in by_co.values():
        ...
        # An account is "properly exhausted" (done) when it has pipeline value
        # OR has tried 3+ CNC contacts OR has 3+ NOI rejections
        e["is_exhausted"] = bool(
            e["mql"] > 0 or e["hot"] > 0 or
            e["cnc"] >= 3 or e["noi"] >= 3
        )
        # needs_exhaust = POC still has work to do on this account
...
def _build_poc_email(first_name: str, accounts: list,
    ...
    # Sort: Fresh (worst) first, then by YtBM desc
    ...
    rows = "".join(
        ...
        + _td_n(health[c]["mql"] + health[c]["hot"], "color:#7b1fa2;" if (health[c]["mql"] + health[c]["hot"]) else "")
        + f'<td {_TD}>{health[c]["last_called"] or "—"}</td>'
        '</tr>'
        for c in accounts
    )
...
def _send_poc_emails(health: dict, tbl_cache: dict, cfg: dict,
    ...
    for owner, accounts in sorted(by_owner.items()):
        # Resolve email: exact match then partial
...
def run(kylas=None, send_email: bool = True) -> dict:
    ...
    send_email=True:  update Airtable + send team digest + per-POC exhaust emails.
    """
    ...
    needs_exhaust_ct = sum(1 for e in health.values() if e["needs_exhaust"])
