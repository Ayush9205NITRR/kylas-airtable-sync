...
def test_get_call_logs_sizes_the_request_off_the_default_ten():
    ...
    seen = {}
    row = {"id": 43843294, "startTime": "2026-08-21T05:00:00.000Z",
           "relatedTo": [{"id": 4383813, "entity": "deal"}]}
    ...
    assert seen["params"]["size"] == KylasClient.CALL_LOG_SIZES[0]


def test_size_never_travels_with_page_because_together_they_404():
    # Measured 2026-08-22: entityId+entityType+size answers, entityId+
    # entityType+page answers, and all four together 404 -- which is the same
    # 404 that produced the original, wrong "this endpoint has no paging".
    seen = {}
    c = KylasClient()
    c._get = lambda path, params=None: seen.update(params or {}) or {
        "content": [], "totalElements": 0}
    c.get_call_logs(4383813, "deal")
    assert "size" in seen and "page" not in seen and "sort" not in seen
