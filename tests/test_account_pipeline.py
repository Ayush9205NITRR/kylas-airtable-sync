...
class _FakeAirtable:
    """Stands in for AirtableClient: records what would be written."""

    instances = []

    def __init__(self, table_name, base_id=None):
        self.table_name, self.base_id = table_name, base_id
        self._cache, self._updates, self.flushed = {}, [], False
        _FakeAirtable.instances.append(self)
...
def _load_account_health(monkeypatch):
    import importlib.util
    os.environ.setdefault("AIRTABLE_PAT", "x")
    os.environ.setdefault("AIRTABLE_BASE_ID", "app_test")
    spec = importlib.util.spec_from_file_location(
        "ah_test", os.path.join(os.path.dirname(os.path.dirname(__file__)),
                                "modules", "06_account_health.py"))
    ah = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(ah)
    return ah


FM_CONTACT = {"id": "Kylas Contact Id",
              "accountPipelineStage": "Account Pipeline Stage (BD)"}


def test_contact_write_stamps_every_contact_with_its_account_stage(monkeypatch):
    ah = _load_account_health(monkeypatch)
    _FakeAirtable.instances = []
    monkeypatch.setattr(ah, "AirtableClient", _FakeAirtable)

    contacts = [{"id": 11, "company": 501}, {"id": 12, "company": 501},
                {"id": 13, "company": 502}]
    ap_map = {"501": {"stage": "SQL (Sales Qualified Lead)", "rank": 1},
              "502": {"stage": "Activation", "rank": 12}}

    fake = _FakeAirtable("Contacts")
    _FakeAirtable.instances = []
    monkeypatch.setattr(ah, "AirtableClient", lambda *a, **k: fake)
    fake._cache = {"11": {"id": "rec11", "fields": {}},
                   "12": {"id": "rec12", "fields": {}},
                   "13": {"id": "rec13", "fields": {}}}

    upd, skp = ah._write_contact_pipeline(contacts, ap_map, FM_CONTACT, "app_test")

    written = {rid: f["Account Pipeline Stage (BD)"] for _, rid, f in fake._updates}
    assert written == {"rec11": "SQL (Sales Qualified Lead)",
                       "rec12": "SQL (Sales Qualified Lead)",
                       "rec13": "Activation"}
    assert (upd, skp) == (3, 0)
    assert fake.flushed


def test_contact_write_skips_rows_already_correct(monkeypatch):
    """~18k contacts — re-writing unchanged rows every run would be wasteful."""
    ah = _load_account_health(monkeypatch)
    fake = _FakeAirtable("Contacts")
    monkeypatch.setattr(ah, "AirtableClient", lambda *a, **k: fake)
    fake._cache = {
        "11": {"id": "rec11", "fields": {"Account Pipeline Stage (BD)": "SQL (Sales Qualified Lead)"}},
        "12": {"id": "rec12", "fields": {"Account Pipeline Stage (BD)": "Stale Value"}},
    }
    contacts = [{"id": 11, "company": 501}, {"id": 12, "company": 501}]
    ap_map = {"501": {"stage": "SQL (Sales Qualified Lead)", "rank": 1}}
    ...
    upd, skp = ah._write_contact_pipeline(contacts, ap_map, FM_CONTACT, "app_test")
    assert (upd, skp) == (1, 1)
    assert [rid for _, rid, _ in fake._updates] == ["rec12"]


def test_contact_write_skips_contacts_missing_from_airtable(monkeypatch):
    ah = _load_account_health(monkeypatch)
    fake = _FakeAirtable("Contacts")
    monkeypatch.setattr(ah, "AirtableClient", lambda *a, **k: fake)
    fake._cache = {"11": {"id": "rec11", "fields": {}}}
    contacts = [{"id": 11, "company": 501}, {"id": 99, "company": 501}]
    ap_map = {"501": {"stage": "Activation", "rank": 12}}

    upd, skp = ah._write_contact_pipeline(contacts, ap_map, FM_CONTACT, "app_test")
    assert (upd, skp) == (1, 1)


def test_contact_write_noop_when_field_not_mapped(monkeypatch):
    """No 'accountPipelineStage' in field_map → write nothing, touch nothing."""
    ah = _load_account_health(monkeypatch)
    called = []
    monkeypatch.setattr(ah, "AirtableClient", lambda *a, **k: called.append(1))
    assert ah._write_contact_pipeline([{"id": 1, "company": 2}], {}, {"id": "Kylas Contact Id"}, "b") == (0, 0)
    assert not called, "must not even construct a client when unmapped"


def test_contact_of_unranked_account_is_blanked(monkeypatch):
    """An account that ranks nothing clears the column rather than leaving it stale."""
    ah = _load_account_health(monkeypatch)
    fake = _FakeAirtable("Contacts")
    monkeypatch.setattr(ah, "AirtableClient", lambda *a, **k: fake)
    fake._cache = {"11": {"id": "rec11", "fields": {"Account Pipeline Stage (BD)": "SQL (Sales Qualified Lead)"}}}
    upd, skp = ah._write_contact_pipeline(
        [{"id": 11, "company": 501}], {"501": {"stage": "", "rank": 0}}, FM_CONTACT, "app_test")
    assert (upd, skp) == (1, 0)
    assert fake._updates[0][2] == {"Account Pipeline Stage (BD)": ""}
